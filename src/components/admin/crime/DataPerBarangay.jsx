...
export default function DataPerBarangay({ totalCasesPerBrgy }) {
    ...
    return (
        <>
            ...
            {
                !totalCasesPerBrgy ? <></> :
                    formattedData.map((count) => (
                        <div className='border-b-2 border-slate-200'>
                            <p className="text-center font-serif text-lg m-2">{count.barangay} : <span className="text-2xl font-bold">{count.total_cases}</span></p>
                            <p className='text-center font-semibold'>Crimes:</p>
                            {
                                count.offenses.map(data =>(
                                    <p className='text-xs text-start'>{data.offense.split(" ").slice(0,5).join(" ")} (<span className='font-bold'> {data.count_per_barangay} </span>)</p>
                                ))
                            }
                        </div>
                    ))
