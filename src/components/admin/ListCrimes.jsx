                ...
                !reportedCrime ? <>Loading...</> :
                    reportedCrime.map((data) => (
                        ...
                        onClick={(e) => sendAwareness(e)}>
                            ...
                            <p id={`offence: ${data.offense} Date: ${getSpecificDate(data.created_at)} Address: ${data.city + ", " + data.barangay}`}><span className="font-semibold">Reported By:</span>  {data.first_name + ", " + data.last_name}</p>
                        </div>
                    ))
            }
        </>
